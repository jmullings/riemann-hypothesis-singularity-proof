...
Trinity + Protocol validator for:
    STEPS/STEP_8/EXECUTION/STEP_08_SIGMA_CURVATURE_BRIDGE.py

Description : Verify sigma-Curvature Bridge
Category    : STEP
...
EXECUTION_FILE  = EXECUTION_DIR / 'STEP_08_SIGMA_CURVATURE_BRIDGE.py'
...
# =============================================================================
# CATEGORY DOCTRINES  (STEP)
# =============================================================================

class V_STEP_08(Infinity_Trinity_Validator):
    """
    STEP validator -- STEP_08 (Verify sigma-Curvature Bridge)

    Doctrine I   : Step Script Executes
    Doctrine II  : Output Markers Present
    Doctrine III : No Interactive Input
    """
    ...
    def _doctrine_I_specific(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            _sp = _ilu.spec_from_file_location("_smod_I", EXECUTION_FILE)
            _m  = _ilu.module_from_spec(_sp)
            sys.modules["_smod_I"] = _m
    ...
    def _doctrine_II_specific(self) -> Tuple[bool, str]:
        print(f"\n  [Doctrine II -- Output Markers Present]")
        src  = EXECUTION_FILE.read_text(encoding="utf-8")
        mrks = ["lambda_star", "LAMBDA_STAR", "x_star", "X_STAR",
                "PASS", "FAIL", "result", "singularity", "curvature",
                "operator", "bridge", "zero", "prime"]
        found = [m for m in mrks if m in src]
        ok = len(found) >= 2
        print(f"    markers found: {found[:5]}")
        print(f"    {'✅ PASS' if ok else '❌ FAIL'}: {len(found)} markers (need >=2)")
        return ok, f"markers={len(found)}"

    def _doctrine_III_specific(self) -> Tuple[bool, str]:
        print(f"\n  [Doctrine III -- No Interactive Input]")
        src = EXECUTION_FILE.read_text(encoding="utf-8")
        ok  = "input(" not in src
        print(f"    interactive input(): {not ok}")
        print(f"    {'✅ PASS' if ok else '❌ FAIL'}: No interactive dependencies")
        return ok, f"interactive={not ok}"

    def run_step_08_trinity(self) -> Tuple[bool, Dict]:
        print("\n" + "=" * 60)
        print("  STEP_08 TRINITY PROTOCOL")
        print("  (Verify sigma-Curvature Bridge)")
        print("=" * 60)
        ...
        print("\n  Trinity -- Verdict")
        print("  " + "-" * 38)
        print(f"  Doctrine I   (Step Script Executes:22) : {'✅' if ok1 else '❌'}")
        print(f"  Doctrine II  (Output Markers Prese:22) : {'✅' if ok2 else '❌'}")
        print(f"  Doctrine III (No Interactive Input:22) : {'✅' if ok3 else '❌'}")
...
def write_report(results):
    ...
    td = results['trinity']
    rows = [
        f"# STEP_08 Trinity Validation Report",
        f"",
        f"**Script:** `EXECUTION/STEP_08_SIGMA_CURVATURE_BRIDGE.py`",
        f"**Category:** STEP  **Description:** Verify sigma-Curvature Bridge",
        f"**Generated:** {ts}",
        f"",
    ...
    rows += [
        f"",
        f"## Category Trinity (STEP) -- Doctrines I-III",
        f"",
        f"| Doctrine | Label | Result | Detail |",
        f"|----------|-------|--------|--------|",
        f"| I   | Step Script Executes | {P(td.get('doctrine_I', {}).get('pass', False))} | {td.get('doctrine_I', {}).get('detail', '')} |",
        f"| II  | Output Markers Present | {P(td.get('doctrine_II', {}).get('pass', False))} | {td.get('doctrine_II', {}).get('detail', '')} |",
        f"| III | No Interactive Input | {P(td.get('doctrine_III',{}).get('pass', False))} | {td.get('doctrine_III',{}).get('detail', '')} |",
        f"",
        f"## Ambient Trinity",
        f"",
    ...
    out = ANALYTICS_DIR / 'TRINITY_REPORT_STEP_08.md'
...
def main() -> int:
    print('=' * 70)
    print('  TRINITY VALIDATOR  STEP_08  --  Verify sigma-Curvature Bridge')
    print('  Target: EXECUTION/STEP_08_SIGMA_CURVATURE_BRIDGE.py')
    print('=' * 70)
    ...
    print('\n' + '=' * 70)
    print('  FINAL VERDICT -- STEP_08_SIGMA_CURVATURE_BRIDGE.py')
    print('=' * 70)
