...
# MKM 9 Tones
MKM_TONES: np.ndarray = np.array([
    137.035999084,           # T₀  α⁻¹ 
    23.14069263277927,       # T₁  e^π
    4.6692016091029907,      # T₂  δ (Feigenbaum)
    np.pi,                   # T₃  π
    np.e,                    # T₄  e
    np.sqrt(5.0),            # T₅  √5
    1.7632228343519051,      # T₆  Ω⁻¹
    PHI,                     # T₇  φ
    1.2020569031595943,      # T₈  ζ(3)
], dtype=float)

TONE_NAMES: List[str] = ['α⁻¹', 'e^π', 'δ', 'π', 'e', '√5', 'Ω⁻¹', 'φ', 'ζ(3)']
N_DIM: int = 9
...
def trinity_torus_vector(T: float) -> np.ndarray:
    ...
    return np.array([frac(T * tone / scale) for tone in MKM_TONES], dtype=float)
...
def phi_resonance_vector(T: float) -> np.ndarray:
    ...
    for k in range(N_DIM):
        resonances[k] = PHI_EXP_WEIGHTS[k] * np.sin(T / MKM_TONES[k])
    return resonances
...
def mkm_canonical_state(T: float) -> np.ndarray:
    ...
    for k in range(N_DIM):
        ...
        state[k] = MKM_TONES[k] * sigma[k] * beta_power / (1.0 + beta_power + 1e-15)
